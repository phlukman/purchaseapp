...
public class FirstPurchaseTest
{
    ...
    public void setup() {
        ...
        book = TaxExemptGood.builder().price(new BigDecimal(BOOK_PRICE)).name("1 book").category(new Books()).build();
        musicCD = OnlySalesTaxableGood.builder().price(new BigDecimal(MUSIC_CD_PRICE)).name("1 music cd").category(new Music()).build();
        chocolateBar = TaxExemptGood.builder().price(new BigDecimal(CHOCOLATE_BAR_PRICE)).name("Chocolate Bar").category(new Foods()).build();
        firstPurchase = new Purchase("First purchase");
