...
public class ThirdPurchaseTest {
    ...
    public void setup() {
        ...
        importedFragance = ImportedGood.builder().price(new BigDecimal(IMPORTED_FRAGANCE_PRICE)).name("Imported Fragance").category(new ImportedFragances()).build();
        fragance = OnlySalesTaxableGood.builder().price(new BigDecimal(FRAGANCE_PRICE)).name("Fragance").category(new Fragance()).build();
        headachePills = TaxExemptGood.builder().price(new BigDecimal(HEADACHE_PILLS_PRICE)).name("Headache pills").category(new MedicalSupplies()).build();
        importedChocolate = ImportedGood.builder().price(new BigDecimal(IMPORTED_CHOCOLATE_PRICE)).name("Imported ChocolateBox").category(new ImportedFoods()).build();

        purchase = new Purchase("Purchase #3");
